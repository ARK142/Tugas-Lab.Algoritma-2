...
void pushHead(struct Node **head, int new_data) {
  ...
  // me-link ke head
  (*head) = new_node;
  // node baru dijadikan sebagai head
}
...
int findPosition(struct Node *head, int find_data) {
  ...
  if (find->data > find_data) {
    return 0;
    // data tersebut lebih kecil dari head
  }
  ...
  return 1;
  // data terdapat di dalam susunan linked list (duplikat)
}
...
void sisip(struct Node *head, int new_data) {
  ...
  if (condition == 0)
  // data baru lebih kecil dari head
  {
    pushHead(&head, new_data);
  }

  else if (condition == 1)
  // terdapat data yang sama (duplikat)
  {
    cout << "terdapat data yang sama";
  }

  else
  // data baru, sisipkan di akhir
  {
